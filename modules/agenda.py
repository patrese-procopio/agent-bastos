...
# Senha do chefe armazenada como hash SHA-256
...
def buscar_missoes_recentes(nucleo: str = None, limite: int = 20) -> list:
    try:
        db = _obter_db()
        # Busca TODAS as missões e filtra localmente
        # Isso evita erros de índice composto no Firestore
        ...
        for doc in todas:
            ...
            # AIPEN = todos veem | nucleo específico = vê o próprio + AIPEN
...
def iniciar_monitoramento(parent_app, intervalo_segundos: int = 60):
    """
    Verifica a cada X segundos se chegou missão nova.
    Chama essa função uma vez no interface.py após criar o app.
    """
...
def _popup_nova_missao(parent_app, quantidade: int, missao: dict):
    """Exibe popup de alerta de nova missão."""
    import winsound
    try:
        # Beep de alerta — frequência 1000Hz por 500ms
    ...
    ctk.CTkLabel(
        popup,
        text=f"🔔  {quantidade} NOVA{'S' if quantidade > 1 else ''} MISSÃO{'ES' if quantidade > 1 else ''}",
        font=("Roboto", 16, "bold"),
        text_color="#ef5350"
    ).pack(pady=(20, 4))
...
def _janela_login(parent, callback_sucesso):
    """Abre janela de login. Se autenticado, chama callback_sucesso()."""
    ...
    ctk.CTkLabel(
        janela,
        text="Área exclusiva do Chefe da AIPEN",
        font=("Roboto", 11),
        text_color="gray"
    ).pack(pady=(0, 20))
    ...
    ctk.CTkButton(
        ...
        fg_color="#1565c0",
        hover_color="#0d47a1"
    ).pack(padx=30, pady=(8, 0), fill="x")


# ─────────────────────────────────────────────
# JANELA — lançar missão (após login)
# ─────────────────────────────────────────────
...
def _abrir_lancamento(nucleo_key: str, parent):
    ...
    ctk.CTkLabel(
        janela,
        text=f"DESTINATÁRIO: {nucleo_nome}",
        font=("Roboto", 13, "bold"),
        text_color="#4fc3f7"
    ).pack(pady=(10, 4))
    ...
    ctk.CTkLabel(
        janela,
        text="⚠️  Assim que concluída, favor dar o pronto ao chefe da AIPEN.",
        font=("Roboto", 11),
        text_color="#ffcc80"
    ).pack(pady=(4, 0))
    ctk.CTkLabel(
        janela,
        text="🦉  Corujas, juntos somos mais.",
        font=("Roboto", 11, "bold"),
        text_color="#a5d6a7"
    ).pack(pady=(2, 10))
    ...
    btn_pub = ctk.CTkButton(
        janela,
        text="📤  Publicar Missão",
        command=_publicar,
        height=42,
        font=("Roboto", 13, "bold"),
        fg_color="#1565c0",
...
# ─────────────────────────────────────────────
# JANELA — visualizar missões (agentes, livre)
# ─────────────────────────────────────────────
...
def _renderizar(missoes, display, btn):
    ...
    if not missoes:
        ...
        for m in missoes:
            ...
            display.insert("end", f"📅 {data_str}  |  🎯 {m.get('nucleo', '—')}\n")
...
# ─────────────────────────────────────────────
# ENTRY POINT — adicionar na sidebar
# ─────────────────────────────────────────────
...
def adicionar_agenda_sidebar(sidebar: ctk.CTkFrame, parent_app):
    ...
    # Lançar missão — exige login do chefe
    ctk.CTkButton(
        sidebar,
        text="🔒  Lançar Missão",
        command=lambda: _janela_login(
            parent_app,
            lambda: _abrir_submenu_lancamento(parent_app)
        ),
        height=40,
        font=("Roboto", 13, "bold"),
        fg_color="#4a148c",
        hover_color="#38006b"
    ).pack(pady=(8, 3), padx=15, fill="x")

    # Visualizar por núcleo — livre para todos
    for key in NUCLEOS:
