...
const GLOBAL_CSS = `
  ...
  .input-missao { width:100%;padding:11px 14px;border-radius:8px;border:1.5px solid #E2E8F0;background:#0B1120;font-size:13px;color:#0F172A;resize:vertical;min-height:100px;outline:none;transition:border-color 0.15s;box-sizing:border-box; }
  .input-missao:focus { border-color:#B45309;background:#FFFBF5; }
  .input-senha { width:100%;padding:10px 14px;border-radius:8px;border:1.5px solid #E2E8F0;background:#0B1120;font-size:13px;color:#0F172A;font-family:'JetBrains Mono','Roboto Mono','Courier New',monospace;outline:none;transition:border-color 0.15s;box-sizing:border-box;letter-spacing:0.1em; }
  .input-senha:focus { border-color:#B45309;background:#FFFBF5; }
  .select-nucleo { width:100%;padding:10px 14px;border-radius:8px;border:1.5px solid #E2E8F0;background:#0B1120;font-size:13px;color:#0F172A;font-family:'JetBrains Mono','Roboto Mono','Courier New',monospace;outline:none;cursor:pointer; }
  ...
  .cal-cell { position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:38px;border-radius:8px;border:1px solid rgba(255,255,255,0.07);background:#1A2236;cursor:pointer;transition:all 0.13s ease;user-select:none; }
...
function ModalLancarMissao({ onFechar, onPublicar }) {
  ...
  return (
    <div className="modal-overlay" style={{ position:"fixed",inset:0,zIndex:10000,background:"rgba(15,23,42,0.65)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center" }}
      ...
      <div className="modal-box" style={{ width:480,background:"#111827",borderRadius:14,boxShadow:"0 24px 64px rgba(0,0,0,0.25)",overflow:"hidden" }}>
        <div style={{ padding:"16px 20px",background:"linear-gradient(135deg,#0F172A,#1E293B)",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
          <div style={{ display:"flex",alignItems:"center",gap:10 }}>
            ...
            <div>
              <div style={{ fontSize:15,fontWeight:800,color:"#1A2236" }}>Lançar Missão</div>
              <div style={{ fontSize:13,color:"#94A3B8",fontFamily:MONO }}>{etapa==="senha"?"Autenticação do Chefe AIPEN":`Destinatário: ${nucleoSel}`}</div>
            </div>
          </div>
        ...
        {etapa === "senha" && !sucesso && (
          <div style={{ padding:"24px" }}>
            <div style={{ display:"flex",alignItems:"center",gap:10,padding:"12px 14px",background:"#FFFBF5",border:"1px solid rgba(251,191,36,0.3)",borderRadius:8,marginBottom:20 }}>
              ...
              <span style={{ fontSize:13,color:"#E8A020",fontFamily:MONO,fontWeight:600 }}>Acesso restrito ao Chefe AIPEN</span>
            </div>
            <label style={{ fontSize:15.6,fontWeight:700,color:"#94A3B8",display:"block",marginBottom:7,fontFamily:MONO }}>SENHA DO CHEFE</label>
            ...
            {erroSenha && <div style={{ marginTop:8 }}><span style={{ fontSize:14.3,color:"#F87171",fontFamily:MONO }}>{erroSenha}</span></div>}
        ...
        {etapa === "missao" && !sucesso && (
          <div style={{ padding:"24px" }}>
            <div style={{ marginBottom:16 }}>
              <label style={{ fontSize:15.6,fontWeight:700,color:"#94A3B8",display:"block",marginBottom:7,fontFamily:MONO }}>NÚCLEO DESTINATÁRIO</label>
            ...
            <div style={{ marginBottom:20 }}>
              <label style={{ fontSize:15.6,fontWeight:700,color:"#94A3B8",display:"block",marginBottom:7,fontFamily:MONO }}>ORDEM / MISSÃO</label>
              ...
              <div style={{ textAlign:"right",marginTop:4 }}>
                <span style={{ fontSize:13,color:mensagem.length>500?"#DC2626":"#94A3B8",fontFamily:MONO }}>{mensagem.length}/500</span>
              </div>
            </div>
            {mensagem.trim() && (
              <div style={{ padding:"12px 14px",background:"#0B1120",border:"1px solid rgba(255,255,255,0.07)",borderRadius:8,marginBottom:16 }}>
                <div style={{ fontSize:13,color:"#94A3B8",fontFamily:MONO,marginBottom:6 }}>Preview — como os agentes verão:</div>
                <div style={{ fontSize:15.6,color:"#94A3B8",lineHeight:1.6 }}>
                  <span style={{ fontSize:11.7,fontWeight:700,padding:"1px 6px",borderRadius:3,background:"rgba(96,165,250,0.12)",color:"#60A5FA",fontFamily:MONO,marginRight:8 }}>{nucleoSel}</span>
                  {mensagem.trim()}
                </div>
                <div style={{ fontSize:13,color:"#E8A020",fontFamily:MONO,marginTop:6 }}>Corujas, juntos somos mais.</div>
              </div>
            )}
        ...
        {sucesso && (
          <div style={{ padding:"36px 24px",display:"flex",flexDirection:"column",alignItems:"center",gap:14 }}>
            ...
            <div style={{ textAlign:"center" }}>
              <div style={{ fontSize:16,fontWeight:800,color:"#F1F5F9",marginBottom:6 }}>Missão Publicada!</div>
              <div style={{ fontSize:15.6,color:"#94A3B8",fontFamily:MONO }}>Todos os agentes do {nucleoSel} foram notificados.</div>
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{ padding:"14px 16px" }}>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10 }}>
          <div style={{ display:"flex",alignItems:"center",gap:8 }}>
            ...
            <span style={{ fontSize:13,fontWeight:700,padding:"2px 8px",borderRadius:3,background:"rgba(234,179,8,0.15)",color:"#EAB308",border:"1px solid rgba(234,179,8,0.25)",fontFamily:MONO }}>{missao.nucleo}</span>
          </div>
          <button onClick={fechar} style={{ background:"transparent",border:"none",cursor:"pointer",color:"#94A3B8",fontSize:14 }}>✕</button>
        </div>
        <p style={{ fontSize:15.6,color:"#CBD5E1",lineHeight:1.6,margin:"0 0 12px" }}>{missao.mensagem}</p>
        <p style={{ fontSize:14.3,color:"#EAB308",fontFamily:MONO,margin:"0 0 12px",fontWeight:600 }}>Por favor, acuse ciência desta missão.</p>
        <button onClick={acusarCiencia} style={{ width:"100%",padding:"9px",borderRadius:7,border:"none",background:"linear-gradient(135deg,#B45309,#92400E)",color:"#FFF",fontSize:15.6,fontWeight:700,cursor:"pointer",fontFamily:MONO }}>Acusar Ciência</button>
      </div>
    </div>
  )
}
...
function BadgeStatus({ status }) {
  if (status === "ciencia") return <span style={{ fontSize:13,fontWeight:700,padding:"2px 7px",borderRadius:3,background:"rgba(74,222,128,0.08)",color:"#4ADE80",border:"1px solid rgba(74,222,128,0.3)",fontFamily:MONO }}>CIÊNCIA</span>
  return <span className="badge-pendente" style={{ fontSize:13,fontWeight:700,padding:"2px 7px",borderRadius:3,background:"rgba(239,68,68,0.10)",color:"#F87171",border:"1px solid rgba(239,68,68,0.3)",fontFamily:MONO }}>PENDENTE</span>
}
...
function MiniCalendario({ missoes, diaAtivo, onDiaClick }) {
  ...
  return (
    <div className="cal-in" style={{ background:"#FFF",border:"1px solid rgba(255,255,255,0.07)",borderRadius:12,overflow:"hidden",boxShadow:"0 4px 16px rgba(0,0,0,0.06)",flexShrink:0 }}>
      <div style={{ padding:"14px 16px",background:"linear-gradient(135deg,#0F172A,#1E293B)",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
        <div style={{ display:"flex",alignItems:"center",gap:10 }}>
          ...
          <div>
            <div style={{ fontSize:14,fontWeight:800,color:"#1A2236" }}>{MESES[mes]} {ano}</div>
            {totalMes>0 && <div style={{ fontSize:15.6,color:"#EAB308",fontFamily:MONO,marginTop:2 }}>{totalMes} missão neste mês</div>}
          </div>
        </div>
        <div style={{ display:"flex",gap:4 }}>
          ...
          <button onClick={() => { setMes(hoje.getMonth());setAno(hoje.getFullYear()) }} style={{ height:28,padding:"0 10px",borderRadius:6,border:"1px solid rgba(234,179,8,0.35)",background:"rgba(234,179,8,0.1)",cursor:"pointer",fontSize:13,fontWeight:700,color:"#EAB308",fontFamily:MONO }}>HOJE</button>
          <button onClick={() => navMes(+1)} style={{ width:28,height:28,borderRadius:6,border:"1px solid rgba(255,255,255,0.12)",background:"rgba(255,255,255,0.06)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center" }}>
      ...
      <div style={{ padding:"12px 14px 14px",background:"#FFF" }}>
        <div style={{ display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3,marginBottom:6 }}>
          {DIAS_SEMANA.map(d => <div key={d} style={{ textAlign:"center",fontSize:13,fontWeight:700,fontFamily:MONO,color:d==="Dom"||d==="Sáb"?"#CBD5E1":"#64748B",padding:"4px 0" }}>{d}</div>)}
        </div>
          ...
          {grade.map((dia,idx) => {
            ...
            return (
              <div key={k} className={classes} onClick={() => onDiaClick(k===diaAtivo?null:k)}>
                <span style={{ fontSize:15.6,fontWeight:ehHoje||ehAtivo?800:temMissao?700:500,fontFamily:MONO,color:corTexto,lineHeight:1,marginBottom:temMissao?5:0 }}>{dia}</span>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:14,flexWrap:"wrap",marginTop:12,paddingTop:10,borderTop:"1px solid #1A2236" }}>
          {[{bg:"#0F172A",border:"none",label:"Hoje"},{bg:"#FEF3C7",border:"1px solid #F59E0B",label:"Com missão"},{bg:"#B45309",border:"none",label:"Selecionado"}].map(({bg,border,label}) => (
            <div key={label} style={{ display:"flex",alignItems:"center",gap:5 }}>
              ...
              <span style={{ fontSize:15.6,color:"#94A3B8",fontFamily:MONO }}>{label}</span>
            </div>
          ))}
          {diaAtivo && <button onClick={() => onDiaClick(null)} style={{ marginLeft:"auto",fontSize:15.6,fontFamily:MONO,color:"#E8A020",background:"transparent",border:"none",cursor:"pointer",textDecoration:"underline",padding:0 }}>Limpar filtro</button>}
        </div>
      </div>
    </div>
  )
...
export default function Agenda({ onNavigate }) {
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",flex:1,height:"100%",overflow:"hidden",background:"#0B1120" }}>
      ...
      <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 20px",borderBottom:"1px solid rgba(255,255,255,0.07)",background:"#111827",flexShrink:0,boxShadow:"0 1px 3px rgba(0,0,0,0.05)" }}>
        <div style={{ display:"flex",alignItems:"center",gap:10 }}>
          <div style={{ width:34,height:34,borderRadius:8,background:"linear-gradient(135deg,#FEF3C7,#FDE68A)",border:"1px solid rgba(251,191,36,0.3)",display:"flex",alignItems:"center",justifyContent:"center" }}>
          ...
          <div>
            <div style={{ fontSize:15,fontWeight:800,color:"#F1F5F9" }}>Agenda Operacional</div>
            <div style={{ fontSize:14.3,color:"#94A3B8",fontFamily:MONO }}>AIPEN — Missões e Ordens do Dia</div>
          </div>
        </div>
        <div style={{ display:"flex",alignItems:"center",gap:10 }}>
          {pendentesCount>0 && <span style={{ fontSize:14.3,color:"#F87171",fontFamily:MONO,fontWeight:700,background:"rgba(239,68,68,0.10)",border:"1px solid rgba(239,68,68,0.3)",padding:"3px 10px",borderRadius:5 }}>{pendentesCount} pendente{pendentesCount>1?"s":""}</span>}
      ...
      <div className="ag-enter" style={{ display:"flex",gap:14,flex:1,overflow:"hidden",padding:16 }}>
        <div style={{ width:248,flexShrink:0 }}>
          <div style={{ background:"#FFF",border:"1px solid rgba(255,255,255,0.07)",borderRadius:10,overflow:"hidden",boxShadow:"0 2px 6px rgba(0,0,0,0.04)" }}>
            <div style={{ display:"flex",alignItems:"center",gap:8,padding:"11px 14px",borderBottom:"1px solid #1A2236",background:"#0B1120" }}>
              ...
              <span style={{ fontSize:14.3,fontWeight:800,color:"#94A3B8",textTransform:"uppercase" }}>Filtrar por Núcleo</span>
            </div>
            <div style={{ display:"flex",flexDirection:"column",gap:3,padding:"10px 12px" }}>
              {["TODOS",...Object.keys(NUCLEOS)].map(n => (
                <button key={n} onClick={() => setNucleoFiltro(n)} style={{ padding:"7px 12px",borderRadius:6,fontSize:15.6,fontWeight:600,border:"1px solid",cursor:"pointer",textAlign:"left",fontFamily:MONO,background:nucleoFiltro===n?"#0F172A":"transparent",color:nucleoFiltro===n?"#FFF":"#475569",borderColor:nucleoFiltro===n?"#0F172A":"#E2E8F0",transition:"all 0.13s" }}>
        ...
        <div style={{ flex:1,display:"flex",flexDirection:"column",gap:10,overflow:"hidden" }}>
          ...
          <div style={{ flex:1,display:"flex",flexDirection:"column",background:"#FFF",border:"1px solid rgba(255,255,255,0.07)",borderRadius:10,overflow:"hidden",boxShadow:"0 2px 6px rgba(0,0,0,0.04)" }}>
            <div style={{ padding:"12px 16px",borderBottom:"1px solid #1A2236",background:"#0B1120",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
              <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                <span style={{ fontSize:15.6,fontWeight:800,color:"#94A3B8",textTransform:"uppercase" }}>Missões Registradas</span>
                {nucleoFiltro!=="TODOS" && <span style={{ fontSize:13,fontWeight:700,padding:"2px 7px",borderRadius:3,background:"rgba(96,165,250,0.12)",color:"#60A5FA",fontFamily:MONO }}>{nucleoFiltro}</span>}
                {labelDiaAtivo && <span style={{ fontSize:13,fontWeight:700,padding:"2px 7px",borderRadius:3,background:"rgba(232,160,32,0.12)",color:"#E8A020",fontFamily:MONO }}>{labelDiaAtivo}</span>}
              </div>
              <span style={{ fontSize:14.3,color:"#94A3B8",fontFamily:MONO }}>{missoesFiltradas.length} resultado{missoesFiltradas.length!==1?"s":""}</span>
            </div>
            <div style={{ flex:1,overflowY:"auto" }}>
              ...
              {!loading && missoesFiltradas.length===0 && (
                <div style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:40,gap:10 }}>
                  ...
                  {diaAtivo && <button onClick={() => setDiaAtivo(null)} style={{ fontSize:15.6,color:"#E8A020",background:"transparent",border:"none",cursor:"pointer",textDecoration:"underline" }}>Ver todas as missões</button>}
                </div>
              )}
              {!loading && missoesFiltradas.map((m,i) => (
                <div key={m.id||i} className="ag-row" style={{ padding:"15px 18px",borderBottom:i<missoesFiltradas.length-1?"1px solid #0B1120":"none",background:m.status==="pendente"||!m.status?"#FFFBF5":"#FFF",borderLeft:`3px solid ${m.nucleo==="AIPEN"?"#16A34A":(m.status==="pendente"||!m.status)?"#DC2626":"#1D4ED8"}` }}>
                  <div style={{ display:"flex",alignItems:"flex-start",gap:12 }}>
                    <div style={{ flexShrink:0,marginTop:2,display:"flex",flexDirection:"column",gap:5 }}>
                      <span style={{ fontSize:13,fontWeight:800,padding:"2px 7px",borderRadius:3,fontFamily:MONO,background:m.nucleo==="AIPEN"?"#F0FDF4":"#DBEAFE",color:m.nucleo==="AIPEN"?"#16A34A":"#1D4ED8",border:`1px solid ${m.nucleo==="AIPEN"?"#86EFAC":"#93C5FD"}` }}>{m.nucleo}</span>
                      <BadgeStatus status={m.status} />
                    </div>
                    <div style={{ flex:1 }}>
                      <p style={{ fontSize:13,color:"#F1F5F9",lineHeight:1.7,margin:0,fontWeight:500 }}>{m.mensagem}</p>
                      <div style={{ display:"flex",alignItems:"center",gap:8,marginTop:6,flexWrap:"wrap" }}>
                        <span style={{ fontSize:14.3,color:"#94A3B8",fontFamily:MONO }}>{formatTs(m.timestamp)}</span>
                        <span style={{ fontSize:13,color:"#CBD5E1" }}>·</span>
                        <span style={{ fontSize:14.3,color:"#E8A020",fontFamily:MONO }}>Corujas, juntos somos mais.</span>
                        {m.status!=="ciencia" && <button onClick={() => acusarCiencia(m)} style={{ marginLeft:"auto",fontSize:14.3,fontWeight:700,padding:"4px 12px",borderRadius:5,border:"1px solid #B45309",background:"transparent",color:"#E8A020",cursor:"pointer",fontFamily:MONO }}>Acusar Ciência</button>}
                      </div>
                    </div>
                  </div>
                </div>
